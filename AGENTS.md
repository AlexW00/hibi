...
- [SettingsView.swift](Hibi/Views/SettingsView.swift) — iOS-Settings-style root list, **pushed onto `ContentView`'s `NavigationStack`** (not a sheet) via `.navigationDestination(isPresented:)`, so it's a full screen with a system back chevron and no inner `NavigationStack` of its own. A **General** section of `NavigationLink` rows (Appearance, Units, Calendars & Reminders) drills into sub-pages on that same stack, then a DEBUG demo-mode toggle, a conditional "Review permissions" CTA (shown only when calendar/location access is missing; calls `dismiss` to pop, and ContentView's `onChange(of: showSettings)` re-presents onboarding once popped), and an **About** section. The About section's "What's New" row carries the version string on its trailing edge (tapping opens the Notelet changelog via `whatsNewVersion`/`.noteletSheet`) and is followed by a More Apps link. Appearance and Units have their own sub-page views (`AppearanceSettingsView`, `UnitsSettingsView`) in the same file; Calendars & Reminders drills into `CalendarSelectionView`.
