...
- [SettingsView.swift](Hibi/Views/SettingsView.swift) — iOS-Settings-style root list: a group of `NavigationLink` rows (Appearance, Units, Calendars & Reminders) that drill into sub-pages, then a DEBUG demo-mode toggle, a conditional "Review permissions" CTA (shown only when calendar/location access is missing; calls the sheet-level `dismiss` so ContentView can re-present onboarding), and a bottom card combining What's New / Version / More Apps. Appearance and Units have their own sub-page views (`AppearanceSettingsView`, `UnitsSettingsView`) in the same file; Calendars & Reminders drills into `CalendarSelectionView`.
