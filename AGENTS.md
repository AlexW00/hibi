...
- [SettingsView.swift](Hibi/Views/SettingsView.swift) — iOS-Settings-style root list: a **General** section of `NavigationLink` rows (Appearance, Units, Calendars & Reminders) that drill into sub-pages, then a DEBUG demo-mode toggle, a conditional "Review permissions" CTA (shown only when calendar/location access is missing; calls the sheet-level `dismiss` so ContentView can re-present onboarding), and an **About** section. The About section's "What's New" row carries the version string on its trailing edge (tapping opens the Notelet changelog via `whatsNewVersion`/`.noteletSheet`) and is followed by a More Apps link. Appearance and Units have their own sub-page views (`AppearanceSettingsView`, `UnitsSettingsView`) in the same file; Calendars & Reminders drills into `CalendarSelectionView`.
