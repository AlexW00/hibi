...
- **DayView refactor (surgical):** inside `paperCard()`, replace the inline fill+border+chrome composition with `PaperSubstrate(texture: style.texture, ruling: style.ruling, fill: AdaptivePalette.paperFill(style.tint, depth: <fractional card depth>), chromeAmount: chromeAmount, cornerRadius: 18)`. **Keep** the tear/stack/parallax orchestration, the shadows, and `PageContent`.
  ...
  - **Border-direction reconciliation (flagged in Task 2):** `PaperSubstrate` scales its border *with* `chromeAmount` (front = full border), but today's DayView uses `edgeHighlightAmount = 1 − chromeAmount` — the hairline edge highlight is strongest on the *receding back* cards (the dark-mode silhouette cue on near-black back cards). When composing `PaperSubstrate`, preserve that: either pass the inverted value or extend `PaperSubstrate` with a separate `edgeAmount` parameter. Do NOT let back cards lose their edge highlight.
  - **Regression guard:** the tear-off feel (drag-to-rip, parallax, progressive depth, back-card edge highlight) must be unchanged for the default (cream/smooth/plain) style — default DayView looks byte-identical to before. On-device only.
